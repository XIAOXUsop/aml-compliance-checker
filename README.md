...
> **源码与已发布版本的范围不同。** 当前 `master` 的 `ed85689` 修复了方法实参、Map 键语义信号与单字符邮箱识别；已发布的 0.4.5 tag 不含这项修复。需要这些修复时先从源码构建，待 0.4.6 正式发布后再下载对应 ZIP。发布门禁会检查 tag 包含该提交，并重新下载上传的 ZIP 逐字节核对。
>
