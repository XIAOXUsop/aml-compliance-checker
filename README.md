...
> **0.4.6 已发布。** 2026-09-26 从公开 Release 下载的 ZIP 为 32813 字节，
> 已核对压缩包、JAR、`plugin.xml` 版本和 IDE 252+ 下限；
> [发布工作流](https://github.com/XIAOXUsop/aml-compliance-checker/actions/runs/36224092920)也已重新下载并逐字节比较。
> 干净 IDEA 的安装和启动冒烟尚待完成。下面是 2026-09-21
...
> **0.4.5 不包含后续修复。** `ed85689` 修复了方法实参、Map 键语义信号与单字符邮箱识别；0.4.6 发布目标提交包含该修复。发布工作流已改为监听 `published`，也可对现有 tag 手动重跑；实际 Release ZIP 已通过版本与结构校验。目标 IDEA 安装冒烟仍待验收。
>
